...
/*
    ...
    - Example Output:
        Array [0]: ZPHV-GAEQ-NYAC-SCOQ
        Array [1]: FGPV-VAUR-MJNR-TOCF
        Array [2]: DSWM-QNMM-FFRH-XJWB
        Array [3]: AMUW-EOYA-LXFB-SHYT
...
struct GeneratedKeys
{
    ...
    char randomChar;
    std::string keys[Constant::MAX_KEYS];
    ...
    void createArray()
    {
        for (int i = 0; i < this->howManyKeys; i++)
        {
            this->keys[i] = getKey();
        }
    }
    ...
    void printKey()
    {
        ...
        {
            std::cout << "Array [" << i << "]: " << this->keys[i] << std::endl;
        }
    }
};
int main()
